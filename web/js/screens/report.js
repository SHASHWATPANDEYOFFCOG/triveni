...
${COMMANDS.map(([cmd, why]) => `${esc(cmd.padEnd(16))}<span class="comment"># ${esc(why)}</span>`).join("\n")}
    ...
    <div class="honesty">
      ...
      <p>
        <strong>Assumed:</strong> the six cost-model parameters — the loaded cost of an
        analyst's hour, the minutes to triage an exception, to unwind a false match and
        to review a flagged one, the share of a wrongly-matched amount never recovered,
        and the minutes to eyeball a row by hand. These are <em>stated, illustrative
        assumptions</em>, not sourced findings; they are labelled as such in
        <code>core/costmodel.py</code>, in the eval output and here. Substitute your own
        and every rupee figure moves with them.
      </p>
      <p>
